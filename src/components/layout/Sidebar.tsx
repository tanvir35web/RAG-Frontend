import { FileText, Layers, MessageSquare, Settings, Upload } from 'lucide-react';
...
export function Sidebar({ activeTab, onTabChange, className }: SidebarProps) {
  ...
  return (
    ...
    >
      {/* Brand */}
      <div className="flex items-center gap-2.5 px-5 py-5 border-b border-gray-200 dark:border-white/[0.06]">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500 to-blue-500 flex items-center justify-center shadow-lg shadow-violet-500/25 flex-none">
          <Layers size={15} className="text-white" />
        </div>
        <div>
          <p className="font-bold text-sm text-gray-900 dark:text-zinc-100 leading-none">
            DocuRAG
          </p>
          ...
          <button
            ...
            className={cn(
              ...
              activeTab === item.id
                ...
                : 'text-gray-600 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-zinc-200 hover:bg-gray-100 dark:hover:bg-white/[0.05]',
            )}
          >
            <span className={cn(
              'flex-none',
              activeTab === item.id ? 'text-violet-600 dark:text-violet-400' : '',
            )}>
              {item.icon}
            </span>
